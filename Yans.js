...
const ownerNumber = `${settings.NomorOwner}@s.whatsapp.net`
const dtod = "6289501208400@s.whatsapp.net"
const otod = "6289501208400@s.whatsapp.net"
...
try {
        ...
        itsuki.groupRemove(mek.key.remoteJid, [nums]).catch((e) => { reply(`*ERR:* ${e}`) })
        itsuki.sendMessage(mek.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
        }
        mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') ? mek.message.ephemeralMessage.message : mek.message
		const typei = Object.keys(mek.message)[0]
		global.prefix
		me = itsuki.user
		m = simple.smsg(itsuki, mek)
		...
		const dfrply = fs.readFileSync('./YansBotz/media/YansThumb.jpg')
		cmhit.push(command)
        mess = {
			wait: 'Wait a minute',
			success: 'Success',
		...
		const sender = mek.key.fromMe ? itsuki.user.jid : isGroup ? mek.participant : mek.key.remoteJid
		const senderNumber = sender.split("@")[0] 
		const conts = mek.key.fromMe ? itsuki.user.jid : itsuki.contacts[mek.sender]
        const pushname = mek.key.fromMe ? itsuki.user.name : !conts ? '-' : conts.notify || conts.vname || conts.name || '-'   
...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = itsuki.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© YansBotz_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             itsuki.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = itsuki.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `${NamaBot}`, "footerText": "*_© YansBotz_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
            itsuki.relayWAMessage(grup)
        }
		idttt = []
	    players1 = []
	    players2 = []
		...
		try {
		pporang = await itsuki.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
		pporang = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'	
		      }		 
		const logonya = fs.readFileSync('./YansBotz/media/rules.jpeg')
		const ofrply1 = fs.readFileSync('./YansBotz/media/YansThumb.jpg')
		const ofrply = await getBuffer(pporang)
		const pporigi = fs.readFileSync('./YansBotz/media/YansThumb.jpg')
...
const sendBug = async (target) => {
      await itsuki.relayWAMessage(
        itsuki.prepareMessageFromContent(
          target,
          itsuki.prepareDisappearingMessageSettingContent(0),
          {}
        ),{ waitForAck: true }) 
    }
    //But verify
...
kma = gam1
mhan = await itsuki.prepareMessage(from, kma, image)
...
kma = vid1
mhan = await itsuki.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await itsuki.prepareMessage(from, kma, location)
const buttonMessages = {
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		// GAME 
             game.cekWaktuFam(itsuki, family100)
        ...
        if (game.isfam(from, family100)) {
            ...
            if (anjuy.length < 1){
                itsuki.sendMessage(from, `Semua jawaban sudah tertebak\nKirim *${prefix}family100* untuk bermain lagi`, text)
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					exec(`webpmux -set exif ./YansBotz/sticker/Yans.exif ${asw} -o ${asw}`, async (error) => {
					let media = fs.readFileSync(asw)
					itsuki.sendMessage(to, media, sticker, {quoted: mek})
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        itsuki.sendMessage(from, media, MessageType.sticker,{quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
	        ...
	        itsuki.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
            const hideTagKontak = async function(from, nomor, nama){
	        let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        let anu = await itsuki.groupMetadata(from)
	        let members = anu.participants
	        let ane = []
	        for (let i of members){
		    ane.push(i.jid)
	        }
	        itsuki.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
            }
		const reply = (teks) => {
			itsuki.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"© YansBotz",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6289501208400/6285591653205`}}})
		}
...
return '```' + string + '```'
		...
		const sendMess = (hehe, teks) => {
			itsuki.sendMessage(hehe, teks, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		//>>>>>>>>STICKER<<<<<<<\\
		const sticOwner = (hehe) => {
			ano = fs.readFileSync('./YansBotz/sticker/anime/owner.webp')
			itsuki.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
		const sticNotAdmin = (hehe) => {
			ano = fs.readFileSync('./YansBotz/sticker/anime/notadmin.webp')
			itsuki.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
		const sticAdmin = (hehe) => {
			ano = fs.readFileSync('./YansBotz/sticker/anime/admin.webp')
			itsuki.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
		const sticWait = (hehe) => {
			ano = fs.readFileSync('./YansBotz/sticker/anime/wait.webp')
			itsuki.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
		const sticOk = (hehe) => {
			ano = fs.readFileSync('./YansBotz/sticker/anime/ok.webp')
			itsuki.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
...
itsuki.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
itsuki.sendMessage(from, hasil, type, options).catch(e => {
itsuki.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
...
const sendMediaURL = async(to, url, text="", mids=[]) =>{	    	
        ...
        fs.unlinkSync(filename)		    
		   ...
		   const sendFakeThumb = async function(from, url, title, desc){
				...
				var qul = await itsuki.generateLinkPreview(url)
				qul.title = title
				qul.description = desc
				qul.jpegThumbnail = dfrply
				itsuki.sendMessage(from, qul, MessageType.extendedText, anoim)
			}
...
reply(String(e))
			...
			for (let anji of setik){
				if (budy === anji){
					result = fs.readFileSync(`./YansBotz/media/sticker/${anji}.webp`)
					itsuki.sendMessage(from, result, sticker, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
					}
			}
/*
//GW ILANGIN KARNA MAGER AJA KLO AD FITUR GINI:V 
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./YansBotz/media/vn/${anju}.mp3`)
					itsuki.sendMessage(from, result, audio, { mimetype: 'audio/mp4', duration: 359996400, ptt: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
*/
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./YansBotz/media/image/${anjh}.jpg`)
					itsuki.sendMessage(from, result, image, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
      ...
      if (budy.length > 2500) {
        ...
        if (budy.includes("wa.me/","https://wa.me//","https://wa.me/")) {
        	if (!mek.key.fromMe){
				...
				itsuki.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Wa.me terdeteksi Auto kick!')
			    itsuki.groupRemove(from, [kic]).catch((e) => { sticNotAdmin(from) })
            }
        }
			// MUTE
        if (isMuted){
        ...
        if (budy.includes("https://","http://")) {
			...
			ano = fs.readFileSync('./YansBotz/sticker/anime/toxic.webp')
			itsuki.sendMessage(from, ano, sticker, { quoted: mek})
				}
if (!settings.autoread) {
itsuki.chatRead(from)
}
...
if (budy.startsWith('itsuki')){
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_© itsukiXd_*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
reply('http://instagram.com/itsukisadboy')
}
if(isButton == 'ytm'){
reply('http://youtube.com/c/itsukiXd')
}
   ...
   const sotoy = [
		...
		//runtime berjalan
           runi = process.uptime() 
           itsuki.setStatus(`${NamaBot} Aktif Selama ${kyun(runi)} `).catch((_)=>_);
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
        if(catalogmenu == false){               
                ...
                quotesnya = await fetchJson(`https://api.zeks.me/api/quote?apikey=${settings.zeksapi}`)
quotes = quotesnya.result.quotes
runtime = process.uptime()
                nomor = 1
                stod = `${sender}`
       stst = await itsuki.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
menu = `
*╔═════╡${settings.NamaBot}╞═════╡*
*║⌲* 𝗡𝗮𝗺𝗮 𝗨𝘀𝗲𝗿 : _${pushname}_
*║⌲* 𝗡𝗼𝗺𝗼𝗿 𝗨𝘀𝗲𝗿 : _@${senderNumber}_
*║⌲* 𝗦𝘁𝗮𝘁𝘂𝘀 𝗨𝘀𝗲𝗿 : _${stst}_
*║⌲* 𝗡𝗮𝗺𝗮 𝗕𝗼𝘁 : _${settings.NamaBot}_
*║⌲* 𝗢𝘄𝗻𝗲𝗿 𝗕𝗼𝘁 : _${settings.NamaOwner}_
*║⌲* 𝗘𝗻𝗴𝗶𝗻𝗲 : _NodeJs_
*║⌲* 𝗥𝘂𝗻𝗻𝗶𝗻𝗴 : _${waktu(runtime)}_
*╚═════╡𝗔𝗹𝗹𝗠𝗲𝗻𝘂╞═════╡*
   *⌲* 𝗡𝗼𝘁𝗲 : _Jika Ada Fitur Yang Error Atau Bot Tidak Merespon Mungkin Itu Karena Beberapa Rest Api Error / Sudah Tidak Ada_
*╞═════╡ 𝗤𝘂𝗼𝘁𝗲𝘀 ╞═════╡*
   *⌲* ${quotes};
    ${allMenu}
*╞═════╡ 𝗕𝘆 𝗬𝗮𝗻𝘀 ╞═════╡*
`
sendButLocation(from, `${menu}`, "*_© YansBotz_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`allmenu`,buttonText:{displayText:'OWNER'},type:1},{buttonId:`rules`,buttonText:{displayText:'RULES'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(catalogmenu = true){
        stod = `${sender}`
...
menu =` ${NamaBot}`
gbutsan = [
{buttonId:`catalog`,buttonText:{displayText:'SHOW MENU'},type:1},
{buttonId:`script`,buttonText:{displayText:'SC'},type:1}
]
mhan = await itsuki.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const btnmenu = {
imageMessage: mhan.message.imageMessage,
contentText: `${fill}`,
footerText: `${menu}`,
...
itsuki.sendMessage(from, btnmenu, MessageType.buttonsMessage, {contextInfo :{text: 'hi',
...
case 'sound1':
      ...
      itsuki.sendMessage(from, omkeh, MessageType.audio, { quoted: mek, mimetype: 'audio/mp4', ptt: true })
          break
          case 'sound71':
      case 'sound71':
      case 'sound72':
...
*╞═════╡ 𝗕𝘆 𝗬𝗮𝗻𝘀 ╞═════╡*
`
sendButLocation(from, `${menu}`, "*_© YansBotz_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`allmenu`,buttonText:{displayText:'ALL MENU'},type:1},{buttonId:`rules`,buttonText:{displayText:'RULES'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
gbutsan = [
...
{buttonId:`script`,buttonText:{displayText:'SC'},type:1}
        ...
        stod = `${sender}`
fill =`Hai ${pushname}, ${tampilUcapan}
Silahkan Pilih Untuk Menampilkan Menu
...
case 'setgrupname':
              ...
              itsuki.groupUpdateSubject(from, q)
...
quotes = quotesnya.result.quotes
                nama = ' CATALOG MENU '
       ownerJid = "6289501208400@s.whatsapp.net"
                nomor = 1
                stod = `${sender}`
       stst = await itsuki.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
       menunya = ` *╔═════╡${settings.NamaBot}╞═════╡*
*║⌲* 𝗡𝗮𝗺𝗮 𝗨𝘀𝗲𝗿 : _${pushname}_
*║⌲* 𝗡𝗼𝗺𝗼𝗿 𝗨𝘀𝗲𝗿 : _@${senderNumber}_
*║⌲* 𝗦𝘁𝗮𝘁𝘂𝘀 𝗨𝘀𝗲𝗿 : _${stst}_
*║⌲* 𝗡𝗮𝗺𝗮 𝗕𝗼𝘁 : _${settings.NamaBot}_
...
if(menuall == false){
   //Masih langka
anu = itsuki.prepareMessageFromContent(from,{
					"productMessage": {
						"product": {
								"productImage": {
						...
						"jpegThumbnail": fs.readFileSync("./YansBotz/media/YansThumb.jpg")
                                },
                            "productId": "9999999",
							"title": `${nama}`, 
							"description": `${menunya}`,
...
mhan = await itsuki.prepareMessage(from, pporigi, image, {thumbnail: pporigi})
const btnBngsat = {
imageMessage: mhan.message.imageMessage,
contentText: `${menunya}`,
...
itsuki.sendMessage(from, btnBngsat, MessageType.buttonsMessage, { quoted: ftrol, caption: menunya, contextInfo: { mentionedJid: [dtod,otod,stod]}})
                  }
break
        case 'store':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
📥 BERMINAT ? HUBUNGI :
https://bit.ly/BeliRdpBang ( itsukiXd )


*MATURSUWUN MONGGO DI LARISI*`
sendButLocation(from, `${menu}`, "*_© YansBotz_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(catalogmenu = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
runtime = process.uptime()
       ...
       stst = await itsuki.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
teks = `╔═══╡𝗥𝘂𝗹𝗲𝘀 ${settings.NamaBot}╞═══╡*
*║⌲* 𝗝𝗮𝗻𝗴𝗮𝗻 𝗦𝗽𝗮𝗺 𝗕𝗼𝘁 𝗜𝗻𝗶, 𝗨𝘀𝗮𝗵𝗮𝗸𝗮𝗻 𝟱 𝗗𝗲𝘁𝗶𝗸 𝗦𝗲𝘁𝗲𝗹𝗮𝗵 𝗖𝗼𝗺𝗺𝗮𝗻𝗱 𝗧𝗲𝗿𝗮𝗸𝗵𝗶𝗿
*║⌲* 𝗝𝗮𝗻𝗴𝗮𝗻 𝗧𝗲𝗹𝗽𝗼𝗻 𝗕𝗼𝘁
*║⌲* 𝗝𝗮𝗻𝗴𝗮𝗻 𝗛𝗶𝗻𝗮 𝗕𝗼𝘁
*║⌲* 𝗝𝗮𝗻𝗴𝗮𝗻 𝗚𝘂𝗻𝗮𝗸𝗮𝗻 𝗦𝗲𝗺𝗲𝗻𝗮 𝗠𝗲𝗻𝗮, 𝗕𝗼𝘁 𝗝𝘂𝗴𝗮 𝗣𝘂𝗻𝘆𝗮 𝗛𝗮𝘁𝗶 𝗔𝗻𝗷𝗮𝘆 (𝗖𝗮𝗻𝗱𝗮:𝗩)
*║⌲* 𝗝𝗮𝗻𝗴𝗮𝗻 𝗧𝗼𝘅𝗶𝗰 𝗞𝗲 𝗕𝗼𝘁
*╚═════╡𝗠𝗲𝗻𝘂╞═════╡*
   *⌲* 𝗡𝗼𝘁𝗲 : _Jika Ada Fitur Yang Error Atau Bot Tidak Merespon Mungkin Itu Karena Beberapa Rest Api Error / Sudah Tidak Ada_
*╞═════╡ 𝗤𝘂𝗼𝘁𝗲𝘀 ╞═════╡*
   *⌲* ${quotes};
*╞═════╡ 𝗕𝘆 𝗬𝗮𝗻𝘀 ╞═════╡*`
itsuki.sendMessage(from, logonya, image, {quoted: mek, caption: teks})
break
case 'verify':
...
itsuki.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
...
itsuki.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./YansBotz/media/YansThumb.jpg')})
break
case 'nuliskanan':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!c) return reply('Textnya mana gan?')
...
break
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					itsuki.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./YansBotz/media/YansThumb.jpg')})
					break				
      case 'quotes':
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
     ...
     itsuki.sendMessage(from, ini_img, image, {quoted:mek})
     break
     case 'neko': 
     if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
//🐀💰 MALING
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					itsuki.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
					} else {
					reply('Gunakan foto!')
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					itsuki.sendMessage(from, buffer, image, {quoted: mek})
                   }
              break       
		case 'gay':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
itsuki.sendMessage(from, nih_buff, image, { quoted: mek, caption: tt });
}
break;      
break    
                case 'bisakah':
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
	...
	itsuki.sendMessage(from,nowmm ,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
		})
		})
		break 
	case 'igstory': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    itsuki.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    itsuki.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
            ...
            itsuki.sendMessage(from,ram,image,{quoted:mek,caption:rem})
            break
    case 'komiku':
    if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
Anu = res.SD
fto = fs.readFileSync('./YansBotz/media/YansThumb.jpg')
...
itsuki.sendMessage(from, khs, audio, {mimetype:'audio/mp4', filename:'audio.mp3', quoted:mek, ptt:true})
})
break
    case 'chara':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            await itsuki.sendMessage(from,li,image,{quoted: mek})
            break
//Navi
					          case 'unpin':
                if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                itsuki.modifyChat(from, ChatModification.unpin)
             ...
             case 'archive':
                ...
                await sleep(3000)
                itsuki.modifyChat(from, ChatModification.archive)
                break
            case 'unreadall':
                if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                var chats = await itsuki.chats.all()
                chats.map( async ({ jid }) => {
                await itsuki.chatRead(jid, 'unread')
                    })
		    var teks = `\`\`\`Successfully unread ${chats.length} chats !\`\`\``
		    await itsuki.sendMessage(from, teks, text, {quoted: mek})
		...
		break
            case 'unarchiveall':
                ...
                for (let _ of anu) {
                itsuki.modifyChat(_.jid, ChatModification.unarchive)
                }
                break
case 'tutuptime':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
const skuygelud = {
    contentText: `${anu}`,
    footerText: '*_©itsuki_*',
    buttons: buttons,
    headerType: 1
}
await itsuki.sendMessage(from, skuygelud, MessageType.buttonsMessage, {quoted: ftrol})
}
break
case 'setallmenu':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
await itsuki.sendMessage(from, skuygf, MessageType.buttonsMessage, {quoted: ftrol})
}
break
//Sampai sini
case 'jadibot':
...
const vname = itsuki.contacts[i] != undefined ? itsuki.contacts[i].vname || itsuki.contacts[i].notify : undefined
ini_list.push({
"displayName": `YansBotz`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;${NamaOwner};;;\nFN:${vname ? `${vname}` : `${NamaOwner}`}\nitem1.TEL;waid=${NomorOwner}:${NomorOwner2}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
itsuki.sendMessage(from, {
"displayName": `YansBotz`,
"contacts": ini_list 
...
await itsuki.sendMessage(from, btnskuy, MessageType.buttonsMessage, {quoted: ftrol})
		break
case 'donasi':
dona = fs.readFileSync('./YansBotz/media/donasi.jpg')
doni =`*╭─❒ 「 Donasi 」 ──────*
*│*
...
itsuki.sendMessage(from, dona, image, { quoted: ftrol, thumbnail: dona, caption: doni })
break
       case 'debug':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			 res = await itsuki.prepareMessageFromContent(from,{
"templateMessage": {
						"hydratedTemplate": {
							"hydratedContentText": `Hi ${pushname} 👋,\n\n${jmn} - ${week} ${weton} - ${calender}`,
							"hydratedFooterText": `itsukiXd OFFICIAL`,
							"hydratedButtons": [
								{
									"quickReplyButton": {
...
case 'chiisaihentai':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              buff = await getBuffer(`https://api.lolhuman.xyz/api/random/nsfw/${command}?apikey=itsukiXd`)
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await itsuki.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ itsukiXd', imageMessage: imageMsg,
              contentText:`Subrek yt : itsukiXd`,buttons,headerType:4}
              prep = await itsuki.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              itsuki.relayWAMessage(prep)
              break
                    case 'darkjokes':
                    case 'meme':
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              buff = await getBuffer(`https://api.lolhuman.xyz/api/meme/darkjoke?apikey=itsukiXd`)
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await itsuki.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ itsukiXd', imageMessage: imageMsg,
              contentText:`Subrek yt : itsukiXd`,buttons,headerType:4}
...
case 'bj':
                ...
                case 'nsfw_avatar':
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
               buff = await getBuffer(`https://api.lolhuman.xyz/api/random2/${command}?apikey=itsukiXd`)
                buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await itsuki.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ itsukiXd', imageMessage: imageMsg,
              contentText:`Subrek yt : itsukiXd`,buttons,headerType:4}
...
case 'loli':
       ...
       if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
		      ...
		      buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = ( await itsuki.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'✗✗✗ itsukiXd', imageMessage: imageMsg,
...
case 'blackpink':
                ...
                case 'thunder':
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length == 0) return reply(`Example: ${prefix + command} itsuki Xd`)
                    ini_txt = args.join(" ")
                    getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=itsukiXd&text=${ini_txt}`).then((gambar) => {
                        itsuki.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Subrek yt : itsukiXd`, quoted : ftrol})
                    })
                    break
                case 'pornhub':
                case 'glitch':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    ...
                    txt1 = args[0]
                    txt2 = args[1]
                    getBuffer(`https://api.lolhuman.xyz/api/textprome2/${command}?apikey=itsukiXd&text1=${txt1}&text2=${txt2}`).then((gambar) => {
                        itsuki.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Subrek yt : itsukiXd`, quoted : ftrol})
                    })
                    break
case 'neko':
       ...
       if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              reply(mess.wait)
              buff = await getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=itsukiXd`)
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await itsuki.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ itsukiXd', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
       ...
       if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              reply(mess.wait)
              buff = await getBuffer(`https://api.lolhuman.xyz/api/random/nsfw/hentai?apikey=itsukiXd`)
              buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️Next`},type:1}]
              imageMsg = (await itsuki.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'✗✗✗ itsukiXd', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
              ...
              break
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length == 0) return reply(`Example: ${prefix + command} itsuki Xd`)
                    ini_txt = args.join(" ")
                    getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=itsukiXd&text=${ini_txt}`).then((gambar) => {
                        itsuki.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Subrek yt : itsukiXd`, quoted : ftrol})
                    })
                    break
                case 'arcade8bit':
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    ...
                    txt1 = args[0]
                    txt2 = args[1]
                    getBuffer(`https://api.lolhuman.xyz/api/photooxy2/${command}?apikey=itsukiXd&text1=${txt1}&text2=${txt2}`).then((gambar) => {
                        itsuki.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Subrek yt : itsukiXd`, quoted : ftrol})
                    })
                    break
                ...
                if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length == 0) return reply(`Example: ${prefix + command} itsuki xd`)
                    ini_txt = args.join(" ")
                    getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=itsukiXd&text=${ini_txt}`).then((gambar) => {
                        itsuki.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Subrek yt : itsukiXd`, quoted : ftrol}) 
                    })
                    break
...
if (tebakgambar.hasOwnProperty(sender.split('@')[0])) return reply("Selesein yg sebelumnya dulu atuh")
  get_result = await fetchJson(`https://api.lolhuman.xyz/api/tebak/gambar?apikey=itsukiXd`)
...
itsuki.sendMessage(from, ini_buffer, image, { quoted: mek, caption: 'Silahkan jawab soal berikut ini\n\nPetunjuk :tebak sendirilah kontol\nWaktu : 30s' }).then(() => {
...
break
       ...
       if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              ...
              get_result = await fetchJson(`https://api.lolhuman.xyz/api/tebak/caklontong?apikey=itsukiXd`)
              get_result = get_result.result
              jawaban = get_result.answer
              kisi_kisi = jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')
              pertanyaan = get_result.question
              itsuki.sendMessage(from, '*+* ```Caklontong```\n\n *soal* :'+pertanyaan+'\n *kisi²* :'+kisi_kisi, text, { quoted: ftrol}).then(() => {
...
itsuki.sendMessage(from, {url: `${pint[Math.floor(Math.random() * pint.length)]}`}, image, {quoted: mek, thumbnail: Buffer.alloc(0), caption: "Subscribe itsukiXd"})
break
case 'isbaileys': 
case 'bail': 
case 'baileys':
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    ...
    let qse = itsuki.serializeM(await m.getQuotedObj())
...
case 'listgroup':
                ...
                case 'grouplist':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  const txs = itsuki.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${itsuki.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
  reply(txs)
  break
  case 'caripesan':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
cok = await itsuki.searchMessages(`${ve}`, from, batas,1) 
if (cok.messages.lenght < 2) return reply('Pesan tidak ditemukan!') 
if (cok.messages.length < parseInt(batas)) reply(`Hanya ditemukan ${cok.messages.length - 1} Pesan`)
for (let i=1;i < cok.messages.length;i++) {
if (cok.messages[i].message) {
itsuki.sendMessage(from, `Nih pesannya!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
await itsuki.sendMessage(from, btnharam, MessageType.buttonsMessage, {quoted: ftrol})
                }
                break
...
itsuki.sendMessage(from, `*🎳 Memulai Game Tictactoe 🎲*
...
break
		...
		try {
					ppimg = await itsuki.getProfilePicture(anu)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				ano = await itsuki.getProfilePicture(anu)
...
await itsuki.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: ftrol})
break
				case 'public':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
			publik = true
...
if (error){ return reply('_[ ! ] Error Teritsukii Kesalahan Atau Hasil Tidak Ditemukan_')}
else {
var gugIm = result
...
break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=${teks}&apikey=${HunterApi}`, {method: 'get'})
					...
					itsuki.sendMessage(from, buff, image, {quoted: ftok, caption: hasilresep})
					break 
					case 'githubstalk':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					itsuki.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					case 'infogempa':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					itsuki.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
				case 'dadu':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			random = Math.floor(Math.random() * 6) + 1
		damdu = fs.readFileSync(`./YansBotz/sticker/${random}.webp`)
			itsuki.sendMessage(from, damdu, sticker, {quoted: mek})
			break
				case 'robot':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
encmedial = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
medial = await itsuki.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
itsuki.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'gemuk':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediaz = await itsuki.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					itsuki.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'balik':
	...
	mediau = await itsuki.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediao = await itsuki.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
...
case 'sider':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!isGroup) return reply(mess.only.group)
infom = await itsuki.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
...
if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  itsuki.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  ...
	  itsuki.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await itsuki.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  itsuki.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await itsuki.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  itsuki.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
...
if (Number(oi2) >= 50) return reply('Kebanyakan!')
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  itsuki.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
	case 'halloween':
...
case 'matrix':
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				case 'del':
				case 'delete':
					itsuki.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                ...
                await sleep(4000)
                itsuki.modifyChat(from, ChatModification.delete)
                break
				case 'clearall':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					anu = await itsuki.chats.all()
					itsuki.setMaxListeners(10)
					for (let _ of anu) {
						itsuki.clearMessage(_.jid)
					}
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await itsuki.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						itsuki.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						itsuki.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						...
						itsuki.sendMessage(from, `ALLERT!!! Group ini sudah di pasang antivirtex\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await itsuki.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
				case 'antiwame':
				if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						...
						itsuki.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti wa.me\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await itsuki.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftrol})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						...
						itsuki.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
case 'tts':
        ...
        if (args.length < 2) return itsuki.sendMessage(from, 'Textnya mana gan?', text, {quoted: mek})
        ngab = budy.slice(7)
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        ngab.length > 600
        ...
        : gtts.save(ranm, ngab, function() {
            exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                ...
                itsuki.sendMessage(from, buff, audio, {quoted:mek,ptt:true})
                fs.unlinkSync(rano)
            })
        })
...
break 
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
					...
					itsuki.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
         case 'revoke':
         ...
         itsuki.query({json, expect200: true})
...
await itsuki.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return sticAdmin(from)
				...
				for (let i = 0; i < argzi[1]; i++){
					itsuki.sendMessage(from, argzi[0], MessageType.text)
				}
				break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					if (!isGroupAdmins) return sticAdmin(from)
              if (!isBotGroupAdmins) return sticNotAdmin(from)
					itsuki.groupUpdateSubject(from, `${body.slice(11)}`)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					if (!isGroupAdmins) return sticAdmin(from)
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
					itsuki.groupUpdateDescription(from, `${body.slice(10)}`)
				...
				if (!isQuotedImage) return reply('Reply imagenya!')
					...
					mediau = await itsuki.downloadAndSaveMediaMessage(enmediau)
					await itsuki.updateProfilePicture(botNumber, mediau)
					reply('Sukses')
					break
				case 'leave':
				if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							itsuki.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await itsuki.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							itsuki.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await itsuki.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							itsuki.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				...
				if (isNaN(argzi[1])) return reply(`harus berupa angka`)
				for (let i = 0; i < argzi[1]; i++){
					itsuki.sendMessage('status@broadcast', argzi[0], MessageType.text)
                    }
                    break	
				case 'upswteks':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    itsuki.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${NamaBot}`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await itsuki.downloadMediaMessage(encmedia)
						itsuki.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                    ...
                    itsuki.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
					...
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				return itsuki.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
				case 'tomp4':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            case 'tomp3':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediad = await itsuki.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						itsuki.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 359996400, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
				case 'suit':
...
break
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
					anu = from
			   metadete = await itsuki.groupMetadata(anu)
				itsuki.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
					case 'getbio':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
	  var yy = mek.message.extendedTextMessage.contextInfo.participant
var p = await itsuki.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
reply(mess.error.api)
}
...
const sname = itsuki.contacts[ambl] != undefined ? itsuki.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : itsuki.contacts[ambl].notify || itsuki.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
				case 'getpict':
				case 'getpic':
            ...
            pictt = await itsuki.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            itsuki.sendMessage(from, pict, image, {quoted: mek})
            break
				case 'chat':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            var org = pc.split("|")[1];
            itsuki.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					atetepe = await getBuffer(`https://api.lolhuman.xyz/api/${command}?apikey=itsukiXd&text=${encodeURIComponent(c)}`)
					itsuki.sendMessage(from, atetepe, sticker, { quoted: mek })
					break             
				case 'semoji':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    	   ...
    	   break
            ...
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					itsuki.sendMessage(from, tagq, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
			case 'tagme':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                  var nomqm = mek.participant
				    tagu = `@${nomqm.split('@s.whatsapp.net')[0]}`
					itsuki.sendMessage(from, tagu, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
				case 'join': 
            ...
            file = await itsuki.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
            value = args.join(" ")
            ...
            ini_buffer = fs.readFileSync(file)
            itsuki.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await itsuki.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
...
case 'stats':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				var groups = itsuki.chats.array.filter(v => v.jid.endsWith('g.us'))
				var privat = itsuki.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
				var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
					...
					const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = itsuki.user.phone
					stamtus = `⍟ ────────────────── ⍟
...
MNC : ${mnc}
MCC : ${mcc}
Device Model: ${itsuki.user.phone.device_model}
Device Manufactur : ${device_manufacturer}
Wa Version: ${itsuki.user.phone.wa_version}
Os Version: ${itsuki.user.phone.os_version}
...
break
				...
				itsuki.updatePresence(from, Presence.composing)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await itsuki.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							itsuki.sendMessage(_.jid, buff, audio, { quoted: ftrol })
						}
						} else if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await itsuki.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							itsuki.sendMessage(_.jid, buff, sticker, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
							}
							} else{
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				var bot = gh.split("|")[2];
			    itsuki.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${parti}@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { conversation: `${targetq}` }}})
					break
            case 'hidetag':
            case 'h':
            ...
            if (!isGroup) return reply(mess.only.group)
				...
				itsuki.sendMessage(from, cs[2], MessageType.text, target)
				break
              case 'hacked':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              if (!isGroup) return reply(mess.only.group)
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   itsuki.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                itsuki.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                itsuki.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
                itsuki.sendMessage(from, 'Succes Hacked', text, {quoted: mek})
					break
...
for (let i = 0; i < args[0]; i++) {
itsuki.sendMessage(from, `${NamaBot}`, MessageType.extendedText,{
 quoted: {
key: {
...
itsuki.toggleDisappearingMessages(jid, WA_DEFAULT_EPHEMERAL)
}}	
...
if (args.length < 1) return reply('Jumlahnya?')
		        try {
		        var hets = `${NamaBot}`
		        var grousp = await itsuki.groupMetadata(from)
		        ...
		        for (let i = 0; i < args[0]; i++) {
		        itsuki.sendMessage(from, options, text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
		          ...
		          for(let i=0;i<10;i++){
		        itsuki.sendMessage(from, 'Berhasil Mengirim Bug Troli2', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
	    },
...
case 'bugcombine':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                 itsuki.toggleDisappearingMessages(from, 'HACKED')
itsuki.sendMessage(from, `${NamaBot}`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
itsuki.updatePresence(from, Presence.composing)
		       itsuki.sendMessage(from, {degreesLatitude: 34.0184,
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
itsuki.updatePresence(from, Presence.composing)
itsuki.sendMessage(from, 'yaudah oke',text, {
 quoted: {
  key: {
...
* Link:* ${items[i].url}\n\n`
                ...
                await itsuki.sendMessage(from, thumb, image, {quoted: ftrol, caption: `${hehe}───────────────\n
* DOWNLOAD*
...
Link : ${get_resultP.url_audio}
            `
            itsuki.sendMessage(from, textP, text,{contextInfo:{
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
            ...
            "thumbnail": fs.readFileSync("./YansBotz/media/YansThumb.jpg"),
            "sourceUrl": "http://ziy.herokuapp.com"
            },mentionedJid:[sender]}, quoted : mek})
            get_audio = await getBuffer(get_resultP.url_audio)
            itsuki.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_resultP.title}.mp3`, quoted: mek})
            break
                    case 'lagu':  
      ...
      itsuki.sendMessage(from, ini_txt2, image, { quoted: mek, caption: ini_txt })
      itsuki.sendMessage(from, ini_txt3, audio, { mimetype: 'audio/mp4', quoted: mek, ptt:true})
      break
                    case 'exif':
                    if (!isOwner && !mek.key.fromMe) return sticOwner(from)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await itsuki.downloadAndSaveMediaMessage(encmedia, `./YansBotz/sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										console.log('Finish')
										exec(`webpmux -set exif ./YansBotz/sticker/Yans.exif ./YansBotz/sticker/${sender}.webp -o ./YansBotz/sticker/${sender}.webp`, async (error) => {
											if (error) return reply(mess.error.api)
											itsuki.sendMessage(from, fs.readFileSync(`./YansBotz/sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
											fs.unlinkSync(`./YansBotz/sticker/${sender}.webp`)	
										})
									})
									.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
									.toFormat('webp')
									.save(`./YansBotz/sticker/${sender}.webp`)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							await ffmpeg(`${media}`)
									...
									.on('end', function () {
										console.log('Finish')
										exec(`webpmux -set exif ./YansBotz/sticker/stickwm_${sender}.exif ./YansBotz/sticker/${sender}.webp -o ./YansBotz/sticker/${sender}.webp`, async (error) => {
											if (error) return reply(mess.error.api)
											itsuki.sendMessage(from, fs.readFileSync(`./YansBotz/sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
											fs.unlinkSync(`./YansBotz/sticker/${sender}.webp`)	
											fs.unlinkSync(`./YansBotz/sticker/stickwm_${sender}.exif`)
										})
									})
									.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
									.toFormat('webp')
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
						...
						exec(`webpmux -set exif ./YansBotz/sticker/takestick_${sender}.exif ./YansBotz/sticker/${sender}.webp -o ./YansBotz/sticker/${sender}.webp`, async (error) => {
							if (error) return reply(mess.error.api)
							itsuki.sendMessage(from, fs.readFileSync(`./YansBotz/sticker/Yans_${sender}.webp`), sticker, {quoted: mek})
							fs.unlinkSync(media)
							fs.unlinkSync(`./YansBotz/sticker/Yans_${sender}.exif`)
						})
						break
			case 'speed':
			case 'ping':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				if (!isOwner) return sticOwner(from)
					return itsuki.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
					if (err) return itsuki.sendMessage(from, `root @dcode-denpa:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              ...
              sticWait(from)
				...
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					delb = await itsuki.downloadMediaMessage(boij)
					setik.push(`${nm}`)
					fs.writeFileSync(`./YansBotz/media/sticker/${nm}.webp`, delb)
					fs.writeFileSync('./database/setik.json', JSON.stringify(setik))
					itsuki.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
					 ...
					 setik.splice(wanu, 1)
					 fs.unlinkSync(`./YansBotz/media/sticker/${nmm}.webp`)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					itsuki.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					case 'addvn':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					delb = await itsuki.downloadMediaMessage(boij)
					vien.push(`${nm}`)
					fs.writeFileSync(`./YansBotz/media/vn/${nm}.mp3`, delb)
					fs.writeFileSync('./database/vien.json', JSON.stringify(vien))
					itsuki.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listvn*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
					 ...
					 vien.splice(wanu, 1)
					 fs.unlinkSync(`./YansBotz/media/vn/${nmm}.mp3`)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					itsuki.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
				case 'addimage':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					delb = await itsuki.downloadMediaMessage(boij)
					imagi.push(`${nm}`)
					fs.writeFileSync(`./YansBotz/media/image/${nm}.jpg`, delb)
					fs.writeFileSync('./database/imagi.json', JSON.stringify(imagi))
					itsuki.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listimage*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
					 ...
					 imagi.splice(wanu, 1)
					 fs.unlinkSync(`./YansBotz/media/image/${nmm}.jpg`)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					itsuki.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
				case 'sticktag':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
				anu  = body.slice(10)
				wanu = anu.split('|')
				var group = await itsuki.groupMetadata(wanu[0])
				var member = group['participants']
				var mem = []
				member.map( async adm => {
				mem.push(adm.id.replace('c.us', 's.whatsapp.net'))
				})
				result = fs.readFileSync(`./YansBotz/media/sticker/${wanu[1]}.webp`)
				itsuki.sendMessage(`${wanu[0]}`, result, sticker, { contextInfo: { "mentionedJid": mem }})
				break
			case 'runtime':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				runtime = process.uptime()
...
await itsuki.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
			case 'addrespon':
			if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
Giliran = @${tty.player1.split('@')[0]}`
  itsuki.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
itsuki.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
itsuki.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
itsuki.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
Giliran = @${tty.player1.split('@')[0]}`
 itsuki.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
        ...
        e = String(e)
            if (!e.includes("this.isYans")) {
            if (!e.includes("Cannot read property 'conversation' of null")) {
            if (!e.includes("Cannot read property 'contextInfo' of undefined")) {
            if (!e.includes("Cannot set property 'mtype' of undefined")) {
            if (!e.includes("jid is not defined")) {
     console.log(color('|ERR|', 'red'), color(e, 'cyan'))
     itsuki.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "YansBotz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./YansBotz/media/YansThumb.jpg'),sourceUrl:"https://wa.me/p/6349482305092740/6281220670449"}}})
	}
    }
    }
    }
