...
switch(bulan1) { 
case 0: bulan1 = 'itsukiuari'; break;
...
CFonts.say('Current version : 1.0.0\nCreator : Yans', {
  font: 'console',
  align: 'left',
  colors: ['yellow']
})
console.log(color(`Note :\n- Jangan Memperjual Belikan Bot Ini Kecuali Creator:v\n- Jangan Menghapus Nama Creator\n- Recode Tambahkan Fitur Ngab Jangan Cuma Ganti Text Doang:V`, 'orange'))
CFonts.say('YansBotz@1.0.0 Is Starting', {
  font: 'console',
  align: 'center',
  colors: ['yellow']
})
console.log(color('Checking Modules...', 'yellow'))
require('./index.js')
nocache('./index.js', module => console.log(color('[YansBotz]', 'orange'),color(`'${module}'`, 'lightgray'), 'File is updated!'))
require('./Yans.js')
nocache('./Yans.js', module => console.log(color('[YansBotz]', 'orange'),color(`'${module}'`, 'lightgray'), 'File is updated!'))

console.log(color('Checking Session..', 'yellow'))
	itsuki.browserDescription = [ `${package.name}@${package.version}`, 'ubuntu', '3.0' ]

	itsuki.on('qr', () => {
	console.log(color('[YansBotz] Scan Qr Code...\n[YansBotz] Qr Akan Berganti Dalam 20 Detik..', 'orange'))
})
console.log(color('Session file Is exist', 'yellow'))
console.log(color('Menghubungkan Ke Whatsapp...', 'yellow'))
	fs.existsSync('./YansBotz.json') && itsuki.loadAuthInfo('./YansBotz.json')
	itsuki.on('connecting', () => {
		console.log(color('[YansBotz]','orange'), color('Menghubungkan..', 'lightgray'))
	})
	itsuki.on('open', () => {
		console.log(color('[YansBotz]','orange'), color(`Bot Sudah Terhubung Jika Ada Masalah Bisa Hubungi Creator..`, 'lightgray'))
	})
    
	// session
	await itsuki.connect({
		timeoutMs: 30 * 1000
	})
	fs.writeFileSync(`./YansBotz.json`, JSON.stringify(itsuki.base64EncodedAuthInfo(), null, '\t'))

	// Baterai
	itsuki.on('CB:action,,battery', json => {
...
const sendButImage = async (id, text1, desc1, gam1, but = [], options = {}) => {
      kma = gam1;
      mhan = await itsuki.prepareMessage(id, kma, MessageType.image);
       buttonMessages = {
      ...
      itsuki.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    }
const sendButLocation = async (id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await itsuki.prepareMessage(id, kma, location)
...
itsuki.on("group-update", async (anu) => {

    metdata = await itsuki.groupMetadata(anu.jid);

    if (anu.announce == "false") {

      teks = `- [ Group Opened ] -\n\n_Group telah dibuka oleh admin_\n_Sekarang semua member bisa mengirim pesan_`;

      itsuki.sendMessage(metdata.id, teks, MessageType.text);
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				ppUrl = await itsuki.getProfilePicture(num)
...
itsuki.on('group-update', async (anu) => {
    ...
    itsuki.on('chat-update', async (message) => {
        require('./Yans.js')(itsuki, message)
